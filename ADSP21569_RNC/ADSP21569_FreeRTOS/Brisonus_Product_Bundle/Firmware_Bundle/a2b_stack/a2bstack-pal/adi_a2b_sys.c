...
#include "../../../Commons_Bundle/ComADAU_1978Common.h"
#include "../../../Commons_Bundle/ComADAU_1979Common.h"

/*============= D E F I N E S =============*/

int adi_a2b_ADAU1979Init(void);
int adi_a2b_ADAU1978Init(void);

static int adi_a2b_ADAU1979PllInit(void);
static int adi_a2b_ADAU1978PllInit(void);
static void adi_a2b_WriteTWIReg(unsigned char Reg_ID, unsigned char Tx_Data);
static unsigned char adi_a2b_ReadTWIReg(unsigned char Reg_ID);
...
struct Config_Table Config_array_ADC1[16] = {
		 ...
		 {ADAU1978_REG_BLOCK_POWER_SAI	,	0xf0}, /* LRCLK high then low,data changes on rising edge   */
		 {ADAU1978_REG_SAI_CTRL0		,	0x5A}, /* 1 BCLK delay, TDM8, 48kHz                         */
		 ...
		 {ADAU1978_REG_BLOCK_POWER_SAI	,	0xff},
...
struct Config_Table Config_array_ADC[16] = {
		 ...
		 {ADAU1979_REG_BLOCK_POWER_SAI	,	0xf0}, /* LRCLK high then low,data changes on rising edge   */
		 {ADAU1979_REG_SAI_CTRL0		,	0x5A}, /* 1 BCLK delay, TDM8, 48kHz                         */
		 {ADAU1979_REG_SAI_CTRL1		,	0x00}, /* slot width 32bit, 24 bit data, 50% duty,MSB first */
		 {ADAU1979_REG_CMAP12			,	0x54},
		 {ADAU1979_REG_CMAP34			,	0x76},
		 {ADAU1979_REG_SAI_OVERTEMP		,	0xf8},
		 ...
		 {ADAU1979_REG_BLOCK_POWER_SAI	,	0xff},
...
static int adi_a2b_ADAU1978PllInit(void)
{
	...
	if(eResult != ADI_TWI_SUCCESS)
	{
   	...
   	adi_a2b_WriteTWIReg(ADAU1978_REG_POWER,0x01);
	adi_a2b_WriteTWIReg(ADAU1978_REG_PLL,0x11);

	status = adi_a2b_ReadTWIReg(ADAU1978_REG_PLL);
	while(delay1--)
	{
		asm("nop;");
	}
...
int adi_a2b_ADAU1978Init(void)
{
	...
	adi_a2b_ADAU1978PllInit();

	for(i=0;i<16;i++)
	{
		adi_a2b_WriteTWIReg(Config_array_ADC1[i].Reg_Add, Config_array_ADC1[i].Value);
		Config_read_ADC[i] = adi_a2b_ReadTWIReg(Config_array_ADC1[i].Reg_Add);
...
static int adi_a2b_ADAU1979PllInit(void)
{
	...
	ADI_TWI_RESULT eResult;
   	...
   	eResult = adi_twi_SetHardwareAddress(adi_twi_hDevice, TARGETADDR_1979);
	if(eResult!=ADI_TWI_SUCCESS)
	{
		REPORT_ERROR("TWI Set Hw address failed 0x%08X\n", eResult);
	}

   	adi_a2b_WriteTWIReg(ADAU1979_REG_POWER, 0x01);
	adi_a2b_WriteTWIReg(ADAU1979_REG_PLL, 0x11);

	status = adi_a2b_ReadTWIReg(ADAU1979_REG_PLL);
	while(delay1--)
	{
		asm("nop;");
	}
...
int adi_a2b_ADAU1979Init(void)
{
	...
	adi_a2b_ADAU1979PllInit();

	for(i=0;i<16;i++)
	{
		adi_a2b_WriteTWIReg(Config_array_ADC[i].Reg_Add,Config_array_ADC[i].Value);
		Config_read_ADC[i] = adi_a2b_ReadTWIReg(Config_array_ADC[i].Reg_Add);
